...
public class DocumentationConfig {
  ...
  /**
   * Sorting all the resources in the swagger yaml I want the controllers and models in a given
   * order
   */
  @Bean
    ...
    return openApi -> {
      var schemas =
          openApi.getComponents().getSchemas().values().stream()
              ...
              .collect(
                  Collectors.toMap(
                      Schema::getName, Function.identity(), (p1, p2) -> p1, LinkedHashMap::new));

      openApi.getComponents().setSchemas(schemas);

      var tags = openApi.getTags().stream().sorted(tagComparator()).collect(Collectors.toList());
