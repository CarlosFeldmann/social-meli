...
/**
 * Base interface to all custom repositories, this is just a utility interface to declare common
 * methods
 *
...
public interface BaseRepository<T, ID> extends JpaRepository<T, ID> {
  ...
  /**
   * Returns if found an entity T, otherwise throw a ResourceNotFoundException that will be handled
   * at a controller adviser
   *
  ...
  default T findByIdOrElseThrow(ID id) {
    ...
    return this.findById(id).orElseThrow(() -> new ResourceNotFoundException(resource, id));
  }
}
